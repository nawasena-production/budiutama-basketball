...
export const sendDeviceVerificationCode = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const verifyDeviceCode = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Tidak terautentikasi.');
  }

  const { userId, code, deviceHash } = data as {
    userId?: string;
    code?: string;
    deviceHash?: string;
  };
  ...
  const storedCode = userData['otp_code'] as string | undefined;
  const expiresAt = userData['otp_expires_at'] as admin.firestore.Timestamp | undefined;

  if (!storedCode || !expiresAt) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Kode tidak ditemukan. Minta kode baru.'
    );
  }
...
function calculateFinalStats(
  ...
  for (const doc of eventDocs) {
    ...
    const pidParts = (e.player_id as string).split('_');
